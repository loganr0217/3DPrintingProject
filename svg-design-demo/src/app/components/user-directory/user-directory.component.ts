...
export class UserDirectoryComponent {
    ...
    selectUser(user:any):void {
      ...
      // Updating the correct permissions
      for(const permissions of ["basic", "dealer", "designer", "production"]) {
        if(this.user[4].includes(permissions)) {
          (<HTMLInputElement>document.getElementById("customSwitch_"+permissions))!.checked = true;
        }
      }
    ...
    addUserPermissions():void {
      ...
      const userId:number = this.user[0];
      let userPermissions:string = "";
      let userPermissionsFormatted:string = "";
      for(const permissions of ["basic", "dealer", "designer", "production"]) {
        if((<HTMLInputElement>document.getElementById("customSwitch_"+permissions))?.checked) {
          userPermissions += permissions+ ";";
          userPermissionsFormatted += "\n- " + permissions;
        }
      }
      if(userPermissions.length > 0) {userPermissions = userPermissions.substring(0, userPermissions.length-1);} 
      
      if (userPermissions != undefined && userId != undefined && userId != -1 && confirm('Are you sure you want to update this user (' + userId + ') to the following permissions?' + userPermissionsFormatted)) {
        this.http.get("https://backend-dot-lightscreendotart.uk.r.appspot.com/updateuserpermissions?email='"+email+"'&password='"+password+ "'&userId=" + userId + "&permissions='" + userPermissions + "'").subscribe(result => {
